...
class BTree
{
   ...
   static BNode *findParent(BNode *ptr,int n)
    ...
    int findDiameter()
    {
        return findDiameter(this->head);
    }
    static int findDiameter(BNode *root)
    {
        ...
        {
            int lchild=findDiameter(root->left);
            
            int rchild=findDiameter(root->right);
            //cout<<lchild<<"  "<<root->data<<" "<<rchild<<endl;

/*
Problem-44 JosephusCircle: N people have decided to elect a leader by arranging
...
int main()
{
    ...
    while(1)
    {
        ...
        printf("1)add_begin 2)add_end 3)print_node 4)count_node 5)delete_pos 6)exit 7)splitList");
        printf("8)joseph circle\033[0m\n");
...
void getJosephPos()
{
    CSLL *p,*q;
    int n,m;
    printf("Enter Number of player N: ");
    scanf("%d",&n);
    printf("Enter M(Mth player get eliminated): ");
    scanf("%d",&m);

    p=q=malloc(sizeof(CSLL));
    p->roll=1;
    for(int i=2;i<=n;i++)
    {
        p->next=malloc(sizeof(CSLL));
        p=p->next;
        p->roll=i;
    }
    ...
    for(int count=n;count>1;n--)
    {
        for(int i=0;i<m-1;i++)
            q=q->next;
        q->next=q->next->next;
    }
    printf("LAST PLAYER STANDING:%d\n",q->roll);
}